    ...
    $('.filters > button').click(function () {
        ...
        $.ajax({
            ...
            success: function (data) {
                for (let i = 0; i < data.length; i++) {
                    let place = data[i];
                    $('places').append('<article><div class="title_box"><h2>' + place.name + '</h2><div class="price_by_night">$' + place.price_by_night + '</div></div><div class="information"><div class="max_guest">' + place.max_guest + '</div><div class="number_rooms">' + place.number_rooms + '</div><div class="number_bathrooms">' + place.number_bathrooms + '</div></div><div class="user"><b>Owner:</b>' + place.user.first_name + place.user.last_name + '</div><div class="description">' + place.description + '</div><div class=reviews><h2>3 Reviews <span class="showReview" data-id=' + place.id + '>show</span></h2><ul></ul></div></article>')
                }
            }
        });
    });
      ...
      $('.reviews span').click(function () {
        if ($(this).attr('class') === 'showReview') {
            ...
            $.ajax({
                ...
                success: function (data) {
                    for (let i = 0; i < data.length; i++) {
                        $('.reviews ul').append('<li>' + data[i] + '</li>');
                    }
                }
            });
        }
        else if ($(this).attr('class') === 'hideReview') {
            $('.reviews ul').empty();
        }
        $(this).toggleClass('showReview hideReview');
      });
  });